...
<script>
    ...
    $(function () {
        ...
        */
         ...
         ** 默认加载
         */
        $('#startime').datetimebox("setValue", GetDateStr(-15));
        $('#endtime').datetimebox("setValue", GetDateStr(1));
        acquireData(GetDateStr(-15), GetDateStr(1), '');
...
//			var key = $('#key').textbox("getValue");
            ...
            window.location.href = path + "/files/exportDateInfo?" + dataurl;
        });
        ...
        */
        function acquireData(startime, endtime, key) {
            ...
            grid = $("#redataGril").datagrid({
                fit: true,
                url: path + "/callbuss/datecallbussdetails?" + dataUrl,
                fitColumns: true,
                striped: true,
                columns: [
                    [
                        {field: 'datatime', title: '日期', width: 100, align: 'center'},
//						{field: 'name', title: '坐席人员名称', width: 100, align: 'center', sortable: true},
    ...
    function openrelation(data) {
        ...
        var dialog = phm.modalDialog({
            ...
            height: 580,
            closable: true,
            url: "datedetails_info.jsp?key=" + data.toString(),
            buttons: [
                {
                    text: '关闭',
                    iconCls: 'icon-cancel',

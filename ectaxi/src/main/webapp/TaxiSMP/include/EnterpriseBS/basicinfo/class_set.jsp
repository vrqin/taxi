...
//				height: 560,
            closable: true,
            url: "classSet_add.jsp?id=" + row.id,
            buttons: [
                {
                    text: '保存',
                    iconCls: 'icon-save',
    ...
    $("#delete").click(function () {
        ...
        $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
            if (r) {
                ...
                var requestURL = path + "/classSet/delClassSetByid?id=" + row.id + "&token=" + token;
    ...
    function acquireData(startime, endtime, key) {
        var dataUrl = "startime=" + startime +
            "&endtime=" + endtime +
            "&order=desc&sort=createtime&key=" + key + "&clientid=" + globab_clientId;
        ;

        grid = $("#redataGril").datagrid({
            fit: true,
            url: path + "/classSet/findClassSetForPageList?" + dataUrl,
            fitColumns: true,
            striped: true,
            columns: [
                [
                    {field: 'classname', title: '班别名称', width: 100, align: 'center', sortable: true},
                    {field: 'starttime', title: '起始时间', width: 100, align: 'center', sortable: true},

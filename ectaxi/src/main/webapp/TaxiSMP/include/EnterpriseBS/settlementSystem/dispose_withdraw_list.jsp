        ...
        */
        function acquireData(key, way, begintime, endtime) {
            ...
            grid = $("#redataGril").datagrid({
                fit: true,
                url: path + "/withdraw/findwithdrawListForWithdrarwStatistics?" + dataUrl,
                fitColumns: true,
                striped: true,
                columns: [
                    [
                        ...
                        {field: 'phone', title: '司机电话', width: 100, align: 'center', sortable: true},
                        {
                            ...
                            formatter: function (value, row, index) {
                                var codes = row["name"];
                                return '<span class="openrelation"  >' + value + ' 元 </span>';
                            }
                        },
                        {
                        ...
                        },
                        {
                            ...
                            formatter: function (value, row, index) {
                                var codes = row["name"];
                                return '<span class="openrelation" >处理提现</span>';
                            }
                        }
                    ]
                ],
...
//					console.info(row);
                    if (globab_flag == "operating") {
                        openrelation(row);
                    } else {

                        openrelationWithdraw(row.name, row.extracttime);
                    }
    ...
    //查看电召单信息
    function openrelationWithdraw(data, time) {
//		console.info(data);
        ...
        var dialog = phm.modalDialog({
            ...
            width: 900,
            height: 600,
            url: "dispose_withdraw_count_list.jsp?account=" + data + "&time=" + time,
            buttons: [
                {
                    text: '取消',
                    iconCls: 'icon-cancel',
...
//		console.info(data);
        var dataUrl = "name=" + data.name + "&amount=" + data.amount + "&cashaccount=" + data.cashaccount + "&extracttime=" + data.extracttime + "&phone=" + data.phone + "&way=" + data.way;
        var dialog = phm.modalDialog({
            title: '处理提现',
            closable: true,
            url: "dispose_withdraw_add.jsp?" + dataUrl,
            buttons: [
                {
                    text: '支付宝',
                    iconCls: 'icon-save',

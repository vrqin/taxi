...
<div id="toolbar">
    <div id="input">
        <span>关键字</span>
        <input id="key" class="easyui-textbox" data-options="width:180" prompt="司机帐号 | 联系电话 | 企业名称">
        <span>&nbsp;在线状态</span>
...
//          ////console.info(row);
        ...
        $("#delete").click(function () {
            ...
            $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
                if (r) {
                    ...
                    var requestURL = path + "/driver/delDriverByid?driverid=" + row.driverid + "&token=" + token + "&clientid=" + clientid;
        ...
        $("#resetPassword").click(function () {
            ...
            $.messager.confirm('确认', '您确认想要重置当前帐号密码？默认密码为"123456"', function (r) {
                if (r) {
                    $.ajax({
                        url: path + "/driver/setPasswordbyDriverid?driverid=" + row.driverid + "&token=" + token + "&clientid=" + clientid,
...
//				height: screenheight,
         ...
         ** 获取数据
         */
        function acquireData(isonline, isstop, key) {
            var dataurl = path + "/successionSet/findSuccessionSetForPageList?startime=&endtime=&isonline=" + isonline +
                "&order=desc&sort=createtime&isstop=" + isstop + "&key=" + key + "&token=" + token;
//			////console.info(dataurl);
            grid = $("#redataGril").datagrid({
                ...
                columns: [
                    [
                        {field: 'carnum', title: '已选车辆数', width: 100, align: 'center', sortable: true},
                        {field: 'starttime', title: '交班起始时间', width: 100, align: 'center'},
                        {field: 'endtime', title: '交班结束时间', width: 100, align: 'center', sortable: true},

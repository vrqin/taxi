...
import com.speedtalk.protocol.order.objs.OrderLogin;
...
public class LoginHandler implements IOrderHandler {
    ...
    public void handler(IoSession session, TSCObject obj) {
        try {
            OrderLogin login = (OrderLogin) obj;
            ...
            TabDriver driver = driverDao.getByAccount(login.getSrcMsID());
            ...
            if (null == driver) {
                ...
                session.write(resp);
                session.close(false);
            } else if (!login.getPwd().equals(driver.getPwd())) {
                resp.setCode(OrderMessConstants.FAILURE);
                resp.setDesc(OrderMessConstants.E02);
                session.write(resp);
                session.close(false);
                ...
                driverDao.updateOnlineStatus(login.getSrcMsID(), KabTeam.ONLINE);
                session.write(resp);
                session.setAttribute(ACCOUNT_SESSION_KEY, login.getSrcMsID());

                KabInfo newKabInfo = KabTeam.getInstance().getKab(login.getSrcMsID());
                if (null == newKabInfo) {
                ...
                try {
                    ...
                    for (KabInfo kabInfo : list) {
                        try {
                            if (!kabInfo.getAccount().equalsIgnoreCase(login.getSrcMsID())) {

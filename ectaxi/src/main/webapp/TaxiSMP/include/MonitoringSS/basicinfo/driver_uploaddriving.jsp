...
<script>
    $(function () {
        ...
        $("#form").attr("action", path + "/file/uploaddriving");
//		alert(window.parent.$("#account").textbox('getValue'));
        ...
        $("#iffsubmit").on('click', function () {
            ...
            var account = window.parent.$("#account").textbox('getValue');
            if (account == "") {
                alert("无法获取用户名,请关闭对话框重新点击修改");
                return;
            }
            $("#account").val(window.parent.$("#account").textbox('getValue'));
            window.parent.$("#account").textbox('textbox').attr('readonly', true);

            window.parent.$("#drivingstute").val(1);
            $("#form").submit();
        });
    });

...
//自定义方法 star
//////////////////
var PHONE_RING_NUM = 0;
var PHONE_STATUS_NUM = 0;
var PHONE_STATUS = false;

function AppendPhone(szStatus) {
    document.getElementById("phone").value = szStatus;
    commonpoint(szStatus);
    // ////console.info("来电号码："+szStatus);
    PHONE_STATUS = true;
}

function PhoneStatus(szStatus) {
    var phst = "";
    if (szStatus == 0) {
        phst = "摘机";
    } else if (szStatus == 1) {
        phst = "挂机";
        ++PHONE_STATUS_NUM;
        DailerStatus(PHONE_STATUS_NUM);
    }
    // ////console.info("phst:"+phst);
    document.getElementById("phonestatus").innerHTML = phst;
...
function PhoneRing() {
    ++PHONE_RING_NUM;
    if (PHONE_RING_NUM == 1) {
        document.getElementById("gainstatus").innerHTML = "---";
    }
    if (PHONE_RING_NUM > 1) {
        var status = "";
        if (PHONE_STATUS) {
        ...
        document.getElementById("gainstatus").innerHTML = status;
    }
    // ////console.info(PHONE_RING_NUM);
}

function PhoneRingCancel() {
    PHONE_RING_NUM = 0;

    // ////console.info(PHONE_RING_NUM);
}

function DailerStatus(szStatus) {
    if (szStatus > 0) {
        document.getElementById("dailerstatus").innerHTML = "已连接";
    }
    document.getElementById("gainstatus").innerHTML = "---";
}

function DailerPlugOut() {
    document.getElementById("dailerstatus").innerHTML = "已拔出";
    document.getElementById("phonestatus").innerHTML = "---";
    document.getElementById("gainstatus").innerHTML = "---";
    PHONE_RING_NUM = 0;
    PHONE_STATUS_NUM = 0;
    PHONE_STATUS = false;
    alert("电话盒已拔出,如重新接入请点击右下方刷新按钮");
}

function DeviceRestart() {
    document.getElementById("dailerstatus").innerHTML = "设备重启,请稍候..";
    document.getElementById("phonestatus").innerHTML = "---";
    document.getElementById("gainstatus").innerHTML = "---";
    PHONE_RING_NUM = 0;
    PHONE_STATUS_NUM = 0;

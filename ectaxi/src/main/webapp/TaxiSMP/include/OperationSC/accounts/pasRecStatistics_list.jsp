...
<script>
    ...
    $(function () {
        ...
        ** 按年面板表格
        */
        function yearData(begintime, endtime) {
            var dataUrl = "sort=id&order=desc&begintime=" + begintime +
                "&endtime=" + endtime;
            $("#year").datagrid({
                url: path + "/passengerTopupStatisticsYear/findPassengerTopupStatisticsYearForPageList?" + dataUrl,
                border: false,
                fitColumns: true,
                striped: true,
                columns: [[
                    {field: "annual", title: "年度", width: 100, align: "center", sortable: true},
                    {field: "topupmoney", title: "充值金额(元)", width: 100, align: "center", sortable: true},
                    {field: "detail", title: "明细", width: 100, align: "center", sortable: true},
                ]],
                idField: 'years',
                sortName: 'years',
                sortOrder: 'desc',
    	...
    	** 按月面板表格
    	*/
        function monthData(begintime, endtime) {
            var dataUrl = "sort=id&order=desc&begintime=" + begintime +
                "&endtime=" + endtime;
            $("#month").datagrid({
                url: path + "/passengerTopupStatisticsMonth/findPassengerTopupStatisticsMonthForPageList?" + dataUrl,
                border: false,
                fitColumns: true,
                striped: true,
                columns: [[
    	...
    	** 按日面板表格
    	*/
        function dayData(begintime, endtime) {
            var dataUrl = "sort=id&order=desc&begintime=" + begintime +
                "&endtime=" + endtime;
            $("#day").datagrid({
                url: path + "/passengerTopupStatisticsDay/findPassengerTopupStatisticsDayForPageList?" + dataUrl,
                border: false,
                fitColumns: true,
                striped: true,
                columns: [[
